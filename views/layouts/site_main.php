...
$img = "http://www.offerndeal.codxplore.com/theme/assets/img/logo-top.png";
if ($method == 'coupon-details') {
    $request = Yii::$app->request->queryParams;
    $dealModel = \app\models\Deals::findOne($request['id']);
    $store = \app\models\Stores::find()
            ->where(['api_store_id' => $dealModel->program_id])
            ->one();
    ...
    $img = isset($store->store_logo) ? $store->store_logo : "";
    if ($deal->image_url != null) {
        $img = $dealModel->image_url;
    }
}
?>
<?php $this->beginPage() ?>
